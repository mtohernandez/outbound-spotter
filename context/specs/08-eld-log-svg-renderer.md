...
1. **Two shadcn primitives installed, not one.** Spec line 48 + the prerequisite list (line 209) claimed `Checkbox` was already shipped in `packages/ui`. It wasn't (`ls packages/ui/src/components/ui/ | grep checkbox` returned empty before the install). Installed both `tabs` and `checkbox` via `npx -y shadcn@latest add tabs checkbox --yes` from `packages/ui`. Both land on the `radix-ui` umbrella import the house style uses (no `@radix-ui/react-tabs` / `@radix-ui/react-checkbox` direct deps were added). Bundle delta still in budget (+9.39 KB gzip vs +15 KB ceiling).

2. **Install ran via `npx -y shadcn@latest`, not `pnpm dlx`.** `pnpm dlx shadcn@latest add tabs` silently truncated the CLI's own output, exited 1, and produced no files. `npx -y shadcn@latest add tabs` worked first try with normal output. Future shadcn installs in this repo should prefer `npx`. Documented for spec 09's potential install (`svg2pdf.js` / `jsPDF` don't go through shadcn so this won't matter there, but it will if a future spec adds another primitive).

3. **ESLint carve-out is two blocks, not a one-line `except` extension.** Spec line 165 implied a single-line edit; the actual minimum is (a) a parallel block targeting `./src/features/log-sheet` with `except: ["./log-sheet", "./trip-planner"]`. The trip-planner block was NOT extended — the existing one-way trip-planner rule still blocks any reverse imports from log-sheet back into it. The two-block shape mirrors the existing carve-out exactly and is the correct shape.

4. **`setSearchParams` uses the function-form updater for defensive merge.** Spec line 164 showed the literal `setSearchParams({ view: v })` which would clobber any future co-existing search params. The implementation uses `setSearchParams((prev) => { const params = new URLSearchParams(prev); …; return params; }, { replace: true })`, also stripping the param entirely on the default `"map"` view so the URL never carries `?view=map` litter. Visible behavior is identical today (no other params exist) but prevents silent regression when a future spec adds another search param.

5. **Tier-3 remarks precedence reads `LogEvent.location`, not `LogEvent.note`.** Spec decision 10's Tier 3 wording was "planner `LogEvent.note` for break / sleeper / restart / fuel events." The TripPlan schema has both `LogEvent.note` (the §395 reason text, e.g. "30-min break (§395.3(a)(3)(ii))") and `LogEvent.location` (the planner-stamped city/state, e.g. "Richmond, VA"). The implementation reads `event.location` for the Remarks city/state surface — that's the field's actual purpose — and falls through to `event.note` only as the very last resort before the em-dash. This matches the spec's intent (city/state in Remarks) better than the literal wording; the test `lookup-event-location.test.ts` "tier 3" case asserts this.

6. **`features/log-sheet/` imports `@clerk/react` directly, not via a re-export.** `daily-log-sheets-strip.tsx` calls `useUser()` from `@clerk/react`. The Bulletproof shared-layer rules allow features to import from external packages directly; this matches the precedent in `apps/web-app/src/components/app-shell/nav-user.tsx`. The renderer uses a structural `DriverNameSource` interface (Pick-shaped) instead of importing Clerk's `UserResource` type so the test mock's narrower shape satisfies the call without SDK-type coupling.

7. **Signature gating uses an HTML checkbox; the SVG `<text>` is the rendered output.** Spec decision 13 already documented this, but the boundary deserved an explicit note: the "I certify" Checkbox + Signature override Input live in HTML (footer block); when `iCertify` is true, the renderer mirrors `signatureName` into an SVG `<text>` element with `font-family: var(--font-display)` italic INSIDE the sheet's main SVG. This means PDF export (spec 09) gets the signature as a vector glyph automatically. The other editable fields (truck, carrier, address, co-driver, shipping doc) currently remain HTML-only; spec 09 must decide whether to snapshot them into SVG `<text>` for export or render a separate export-mode component.

8. **Sub-agent ui-visual-validator surfaced two MAJOR a11y findings, both resolved before PR.** M-1: the "I certify" Checkbox at `size-5` (20×20 px) fell short of WCAG 2.5.8's 24×24 px minimum — bumped to `size-6`. M-2: the signature caption SVG `<text>` at `opacity={0.6}` failed APCA Lc 60 in dark mode (Lc 52.6 measured) — raised to `opacity={0.75}` (Lc 67 dark / Lc 82 light, both pass). Also added `motion-safe:` prefix to the `transition-colors` classes on editable inputs so `prefers-reduced-motion` is honored. All three fixes landed before the PR opens.
