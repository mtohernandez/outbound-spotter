...
## Decisions amended post-implementation (phase 1, 2026-05-21)

Sub-agent reviews on branch `feat/10a-trip-exports-backend` (django-pro, python-pro, security-auditor, architect-review, code-reviewer) drove four code amendments to the shape described above in decisions 19–21. These changes shipped in commits `71d513c` + `558a04f`; the spec text is updated below so phase-2 / phase-3 implementers see the live contract.

- **a) `TripExportManager.for_trip` dropped (django-pro m2).** Decision 19 promised the manager exposes `.for_trip(trip_id)` for a future cleanup path. No consumer landed in phase 1 (YAGNI per `context/code-standards.md` line 9); the method was removed. **Phase-3 note**: if the FE cleanup-orphaned-exports flow needs a server-side delete-all-for-trip endpoint, add `.for_trip` back in that spec with its consumer.
- **b) `TripExportResponseSerializer` collapsed into `TripExportListItemSerializer` (python-pro MIN-3).** Decision 20 promised a separate response serializer for POST 201; in practice the shape is identical to the list-item read. Direct use of `TripExportListItemSerializer` keeps the OpenAPI schema tidy (one component, not two with the same fields). **Phase-2 note**: the FE Zod schema for the create-response is the same as the list-item schema; there is no separate response shape to model.
- **c) `transaction.atomic` removed from POST create (django-pro m3).** Decision 20 said the row write runs inside `transaction.atomic` for parity with `services.plan_trip`. A single `INSERT` is autocommit-wrapped; the SAVEPOINT added no rollback semantics. The wrap was decorative and removed. **If a future change adds a second write** (e.g., emitting an audit-log row alongside the create), re-add the wrap explicitly — do not assume it's there.
- **d) Snake↔kebab mode translation lifted onto `ExportMode` (django-pro M1 + python-pro MIN-2).** Decision 19 described "the serializer translates" without specifying the mechanism. The implementation adds two classmethods on `ExportMode`: `to_wire(db_value)` returns `db_value.replace("_", "-")` and `from_wire(wire_value)` does the inverse. Derived programmatically from `ExportMode.values` so adding a third mode (e.g. `compact_grid`) is a single enum-member edit with no serializer changes. **Phase-2 + 3 note**: the FE never sees snake_case and the DB never sees kebab-case; the wire contract is unambiguously kebab-case in/out.

**Known v2 hardening (security-auditor M2, deferred)**: the POST endpoint has no per-(user, trip, mode) dedup. A client can spam the endpoint up to `export_create=60/hour` (1,440 rows/day/user worst case). Acceptable for v1 — small absolute volume — but if utilization runs hotter than expected, add either (a) a dedup window ("last export of this (trip, mode) was <60s ago → return 200 with existing row") or (b) `unique_together` on a minute-bucketed key. Tracked in `progress-tracker.md` Next Up.
