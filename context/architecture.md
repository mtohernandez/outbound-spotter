...
| web-apex              | Vite 8.0.x + React 19.2.x + TypeScript 6.0.x                                                | Apex-domain redirector (signed-out → accounts., signed-in → app.) via Clerk satellite-domain handshake                                                                                                                                                                                                              |
| web-api               | Django 5.2 LTS + Django REST Framework 3.17.x on Vercel Python runtime / Fluid Compute      | Trip CRUD, HOS planner, ORS proxy, JWT verification. LTS chosen over the latest non-LTS stable for the multi-year support window (through 2028-04). Vercel auto-detects Django via `manage.py` and reads `WSGI_APPLICATION` from the prod settings — source <https://vercel.com/docs/frameworks/full-stack/django>. |
| Database              | PostgreSQL 17 (Docker dev, **Neon** managed Postgres prod, free tier) + psycopg 3.3.x       | Users, Trips, Stops, LogEvents. Neon's free tier provides 0.5 GB storage / 100 CU-hours per month with 5-min scale-to-zero — `web_api/settings/prod.py` honours the Neon-Django guide (sslmode=require, CONN_MAX_AGE=240, CONN_HEALTH_CHECKS=True, DISABLE_SERVER_SIDE_CURSORS=True).                               |
...
| Deployment            | **Vercel** (web-app, web-auth, web-apex, web-api) + **Neon** (Postgres)                     | Free-tier hosting. Vercel git integration auto-deploys each project on `main` push; no GitHub Actions deploy workflow required.                                                                                                                                                                                     |
...
- `apps/web-auth/` — Clerk-rendered auth pages styled with shadcn blocks. Issues the Clerk session that both web-app and web-api consume. Deployed at the `accounts.<host>` tenant subdomain. In Clerk's satellite topology this is the **primary** instance — `<ClerkProvider allowedRedirectOrigins={[web-app, web-apex]}>` accepts the redirect handshake from the satellite tenants. Owns nothing trip-related.
- `apps/web-apex/` — Minimal Vite + React + Clerk-satellite app deployed at the bare apex `<host>`. One component (`<Redirector />`) reads `useAuth().isSignedIn` and `window.location.replace`s to `app.<host>` (signed in) or `accounts.<host>/sign-in` (signed out). The Clerk satellite-domain handshake (free in Clerk dev mode) carries the session decision across `.vercel.app` subdomains that aren't cookie-shareable natively. No router, no API calls, no persistence — the page exists only long enough to redirect.
- `apps/web-api/` — Django + DRF service. Owns the HOS planner (a pure Python module under `web_api/hos/`), the ORS client, all DB writes, and JWT verification via Clerk's JWKS. **Hosted on Vercel's Python runtime / Fluid Compute** (verified at <https://vercel.com/docs/frameworks/full-stack/django>, last updated 2026-04-09). Migrations apply via `[tool.vercel.scripts].build = "python manage.py migrate --noinput"` in `pyproject.toml`; `collectstatic` is run automatically by Vercel and the result is served from the Vercel CDN.
...
## Deployment topology (v0.1.0)

Four Vercel projects + one Neon project. Free tier across the board. The four production URLs are documented in `README.md#Production` and reproduced here for invariant-pin:

| Surface  | Hostname                                       | Vercel project name         |
| -------- | ---------------------------------------------- | --------------------------- |
| web-app  | `https://outbound-spotter-app.vercel.app`      | `outbound-spotter-app`      |
| web-auth | `https://outbound-spotter-accounts.vercel.app` | `outbound-spotter-accounts` |
| web-apex | `https://outbound-spotter.vercel.app`          | `outbound-spotter`          |
| web-api  | `https://outbound-spotter-api.vercel.app`      | `outbound-spotter-api`      |

Each project's `vercel.json` declares its framework preset + `rootDirectory` (`apps/<name>`) + `buildCommand` (`cd ../.. && pnpm exec turbo run build --filter=@outbound/<name>`). Vercel's git integration auto-deploys each project on `main` push and builds previews on PRs — there is no GitHub Actions deploy workflow.

**Clerk satellite topology.** `.vercel.app` is on the Public Suffix List, so each subdomain is its own eTLD+1 and cookies do not propagate. The four origins use Clerk's satellite-domain handshake to share session state:

- **Primary** — `outbound-spotter-accounts.vercel.app` (the auth tenant). `<ClerkProvider allowedRedirectOrigins={[VITE_APP_URL, VITE_APEX_URL]}>` accepts the handshake from the satellites.
- **Satellites** — `outbound-spotter-app.vercel.app` and `outbound-spotter.vercel.app`. Each runs `<ClerkProvider isSatellite domain={own URL} signInUrl={primary/sign-in}>` and Clerk handles the redirect dance on `useAuth()` load.
- **Free in Clerk dev mode** (cite `https://clerk.com/docs/guides/dashboard/dns-domains/satellite-domains` — production-mode satellites require a paid plan; v0.1.0 ships on dev keys and the README documents the trade-off).

**Serverless implications (Vercel Python runtime on free Hobby).** Free allowance: 1M invocations + 4 CPU-hours + 360 GB-hrs memory per month, with billing paused during I/O wait per Fluid Compute docs (`https://vercel.com/docs/functions/usage-and-pricing`, 2026-02-18). No background workers (no Celery / cron) — v1 doesn't need them. Function timeout 10 s on free; the longest happy-path request (trip create with cold ORS) stays comfortably under. Bundle limit 500 MB; Django + DRF + Clerk + psycopg ≪ that.

**Neon (managed Postgres free tier).** 0.5 GB storage, 100 CU-hours/month, 10 branches, scale-to-zero after 5 min idle (not disable-able). Always use the **pooled** connection string (hostname contains `-pooler`). Cold start on idle wake adds 1–2 s to the first request after a quiet period — documented in the README.
...
- Wired via `REST_FRAMEWORK["DEFAULT_THROTTLE_CLASSES"]` + `DEFAULT_THROTTLE_RATES`:
  ...
  - `geocode_reverse = 30/min` (`GeocodeReverseView`, spec 11b). Powers the "Use my current location" UX — gesture-bursty (one click → one call), sits between autocomplete (high-volume typeahead) and search (committal lookup). **POST, not GET** — driver coordinates are PII and a GET would log `?lat=&lon=` to every proxy access log (spec 11 follow-up, security MEDIUM-3).
  - `pelias_global = 900/day` (`PeliasGlobalThrottle`, spec 11 follow-up). Tenant-wide singleton bucket stacked on top of the per-user throttles across every Pelias view (autocomplete + search + reverse). Tuned under the HeiGIT 1000/day Pelias quota so we surface a clean DRF 429 before the upstream — prevents one misbehaving user from draining the shared quota for everyone (security-auditor MEDIUM-2).
